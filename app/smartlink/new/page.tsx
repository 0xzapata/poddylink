...
import { PlusCircle, Trash2, Link as LinkIcon, Check,  Edit } from "lucide-react"
...
function EditDialog({ isOpen, onClose, onSave, initialData, isMainLink = false }) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{isMainLink ? 'Edit Main Link' : 'Edit Episode Link'}</DialogTitle>
        </DialogHeader>
        {isMainLink ? (
          <>
            <Input
            ...
            />
            <Select
              value={editData.type || 'spotify'}
              onValueChange={(value) => setEditData({ ...editData, type: value as LinkType })}
            >
              <SelectTrigger>
          ...
          <>
            <Input
              value={editData.episodeName || ''}
              onChange={(e) => setEditData({ ...editData, episodeName: e.target.value })}
            ...
            />
            <Select
              value={editData.icon || 'website'}
              onValueChange={(value) => setEditData({ ...editData, icon: value as LinkType })}
            >
              <SelectTrigger>
...
export default function SmartlinkBuilder() {
  ...
  const editMainLink = (index: number) => {
    setEditingLink({ index, isMainLink: true })
    setIsEditDialogOpen(true)
  }

  const editLink = (index: number) => {
    setEditingLink({ index, isMainLink: false })
    setIsEditDialogOpen(true)
  }

  const handleEditSave = (editedData: Partial<LinkData>) => {
    if (editingLink?.isMainLink) {
      const newMainLinks = [...formData.mainLinks]
      newMainLinks[editingLink.index] = { ...newMainLinks[editingLink.index], ...editedData }
      setFormData(prev => ({ ...prev, mainLinks: newMainLinks }))
    } else if (editingLink) {
      const newLinks = [...formData.links]
      newLinks[editingLink.index] = {
        ...editedData,
        episodeName: editedData.episodeName || '',
        url: editedData.url || '', // Ensure url is always provided
        icon: editedData.icon || 'website'
      }
  ...
  const publishSmartlink = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to publish smartlink:', errorData.error);
      }
    } catch (error) {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <EditDialog
        ...
        initialData={editingLink ? (editingLink.isMainLink ? formData.mainLinks[editingLink.index] : formData.links[editingLink.index]) : null}
        isMainLink={editingLink?.isMainLink}
      />
    </div>
  )
}